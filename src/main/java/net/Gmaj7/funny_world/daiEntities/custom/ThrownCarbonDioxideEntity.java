...
public class ThrownCarbonDioxideEntity extends ThrowableItemProjectile {
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        OxygenRemove(pResult.getEntity().blockPosition());
        this.discard();
    }
    ...
    protected void onHitBlock(BlockHitResult pResult) {
        ...
        OxygenRemove(pResult.getBlockPos().relative(pResult.getDirection()));
        this.discard();
    }
    ...
    private void OxygenRemove(BlockPos blockPos){
        List<LivingEntity> list = this.level().getEntitiesOfClass(LivingEntity.class, new AABB(blockPos).inflate(4));
        for (LivingEntity target : list){
            if(target.isEyeInFluidType(NeoForgeMod.WATER_TYPE.value()))
                target.setAirSupply(target.getAirSupply() - 75);
        }
    }
}
