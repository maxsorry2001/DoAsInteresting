...
public class daiBlocks {
    ...
    public static final DeferredBlock<Block> WIND_BLOCK = DAI_BLOCKS.register("wind_block",
            () -> new WindBlock(BlockBehaviour.Properties.of().mapColor(DyeColor.WHITE).sound(SoundType.SNOW).noCollission().isViewBlocking((blockState, blockGetter, blockPos) -> false).noCollission()));

    public static final DeferredBlock<Block> WIND_TNT = DAI_BLOCKS.register("wind_tnt",
            () -> new WindTNT(BlockBehaviour.Properties.of().mapColor(DyeColor.WHITE).sound(SoundType.SNOW).noCollission().isViewBlocking((blockState, blockGetter, blockPos) -> false).noCollission()));
